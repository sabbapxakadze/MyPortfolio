...
export function CombinedTerminalSkills({ technicalSkills, softSkills }: CombinedTerminalSkillsProps) {
  const [displayedCommands, setDisplayedCommands] = useState<{ command: string; skill: Skill; category: string }[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  
  const allSkills = [
    ...technicalSkills.map(skill => ({ ...skill, category: 'technical' as const })),
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      if (currentIndex < allSkills.length) {
        ...
        const command = skill.category === 'technical' 
          ? `$ npm install ${skill.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')} --save`
          : `$ execute --skill "${skill.name}" --level ${skill.level}`;
        
        setDisplayedCommands(prev => [...prev, { command, skill, category: skill.category }]);
      ...
      <CardContent className="p-4 bg-gray-950 min-h-[400px]">
        <div className="space-y-2">
          <div className="text-gray-400 mb-4">
            <span className="text-blue-400">~/developer$</span> cat skills-overview.txt
          </div>
          ...
          {displayedCommands.map((item, index) => (
            <div key={index} className="space-y-1">
              ...
              <div className="text-green-400 text-xs flex items-center justify-between">
                <span>✓ {item.skill.name} ({item.skill.level}) installed successfully</span>
                <Badge 
                  variant="outline" 
                  className={`${getCategoryColor(item.category)} border-current text-xs px-2 py-0`}
                >
                  {item.category}
                </Badge>
              </div>
          ...
          {currentIndex >= allSkills.length && (
            <div className="mt-6 pt-4 border-t border-gray-800">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <div className="text-cyan-400 text-sm mb-2 font-semibold">🔧 Technical Skills</div>
                ...
                <div>
                  <div className="text-purple-400 text-sm mb-2 font-semibold">💡 Soft Skills</div>
                  <div className="space-y-1">
              ...
              <div className="mt-4 text-gray-500 text-xs">
                <span className="text-blue-400">~/developer$</span> All systems operational ✨
                <div className="text-green-400 mt-1">
                  Status: {technicalSkills.length} technical skills + {softSkills.length} soft skills loaded
                </div>
              </div>
            </div>
          )}
