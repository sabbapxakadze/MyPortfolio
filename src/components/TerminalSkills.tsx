...
export function TerminalSkills({
  ...
  const getPromptSymbol = () =>
    terminalType === "technical" ? "~/projects$" : "~/skills$";
      ...
      <CardContent className="p-4 bg-gray-950 min-h-[300px]">
        <div className="space-y-2 text-green-400">
          <div className="text-gray-400">
            <span className="text-blue-400">{getPromptSymbol()}</span> cat{" "}
          ...
          {displayedCommands.map((command, index) => (
            <div key={index} className="space-y-1">
              ...
              <div className="text-green-400 text-xs">
                ✓ {skills[index]?.name} ({skills[index]?.level}) installed
                successfully
              </div>
            </div>
          ))}
          ...
          {currentIndex >= skills.length && (
            <div className="mt-4 pt-4 border-t border-gray-800">
              ...
              <div className="mt-3 text-gray-500 text-xs">
                <span className="text-blue-400">{getPromptSymbol()}</span> All
                systems operational ✨
              </div>
            </div>
          )}
